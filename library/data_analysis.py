...
class StockAnalysis:
    ...
    def plot_price_history(self, symbol, start_date=None, end_date=None, 
                           show_volume=True, save_path=None):
        """
        ...
        if len(df) >= 50:
            ...
            ax1.plot(df['date'], df['MA50'], 'r-', linewidth=1.5, alpha=0.8, label='50-Day MA')
            
        if len(df) >= 200:
            df['MA200'] = df['close'].rolling(window=200).mean()
            ax1.plot(df['date'], df['MA200'], 'g-', linewidth=1.5, alpha=0.8, label='200-Day MA')
        ...
        if show_volume:
            ...
            ax2.yaxis.set_major_formatter(FuncFormatter(lambda y, _: f'{y/1e6:.0f}M' if y < 1e9 else f'{y/1e9:.1f}B'))
    ...
    def plot_returns_distribution(self, symbol, start_date=None, end_date=None,
                                 period='daily', save_path=None):
        """
        ...
        if period == 'daily':
            ...
            logger.error("Invalid period: %s. Use 'daily', 'weekly', or 'monthly'", period)
            return None
        ...
        try:
            ...
            if len(returns) >= 30:
                ...
                plt.title(f'{period_name} Returns Distribution for {symbol} (μ={mu:.4f}, σ={std:.4f})')
            else:
                plt.title(f'{period_name} Returns Distribution for {symbol}')
        except ImportError:
            logger.warning("scipy.stats not available, skipping normal distribution fit")
            plt.title(f'{period_name} Returns Distribution for {symbol}')
    ...
    def create_performance_dashboard(self, symbol, start_date=None, end_date=None, 
                                    save_path=None):
        """
        ...
        if not valid_prices.empty:
            ...
            years = max(days / 365.25, 0.01)  # Use 365.25 for leap years, minimum of ~3.65 days
        ...
        fig.text(0.5, 0.01, "Note: Returns shown are price returns only (excluding dividends)",
    ...
    def compare_stocks(self, symbols, start_date=None, end_date=None, 
                      normalized=True, save_path=None):
        """
        ...
        for symbol in comparison_df.columns:
            ax.plot(comparison_df.index, comparison_df[symbol], linewidth=2, label=symbol)
        ...
        stats_text = "Total Return:\n" + "\n".join([f"{symbol}: {ret:.2%}" for symbol, ret in sorted_returns.items()])
    ...
    def get_distinct_values(self, column_name, table_name="stock_prices"):
        ...
        query = text(f"SELECT DISTINCT {column_name} FROM {table_name} ORDER BY {column_name} ASC")
        ...
        try:
            ...
            logger.error("Database error getting distinct values for %s: %s", column_name, str(e))
            return []
        except (ValueError, TypeError) as e:
            logger.error("Data error getting distinct values for %s: %s", column_name, str(e))
            return []